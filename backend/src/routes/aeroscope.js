...
router.get('/map', async (req, res) => {
  ...
  const [rows] = await pool.query(`SELECT * FROM aeroscope WHERE ${where}`, params)
  res.json(rows)
})
...
router.get('/export-csv', async (req, res) => {
  ...
  const [rows] = await pool.query(
    `SELECT * FROM aeroscope WHERE ${where} ORDER BY time_start DESC`,
    params
  )
...
router.get('/export-kmz', async (req, res) => {
  ...
  res.setHeader('Content-Disposition', 'attachment; filename="aeroscope.kmz"');
  res.send(kmzBuffer);
});
