        ...
        <table class="w-full text-sm">
          <thead class="bg-slate-50">
            <tr class="text-left border-b">
              ...
              <th class="py-3 px-4 max-w-[160px] truncate">Entidad</th>
          ...
          <tbody>
            <tr v-for="r in items" :key="r.id" class="border-b last:border-0 hover:bg-slate-50/60">
              ...
              <td class="py-3 px-4">{{ r.equipo }}</td>
              <td class="py-3 px-4 font-medium max-w-[160px] truncate whitespace-nowrap" :title="r.entidad">  {{ r.entidad }}</td>
              <td class="py-3 px-4">
...
const fetchData = async (p = 1) => {
  page.value = p
  const { data } = await http.get('/ffpp', {
...
const submitForm = async () => {
  if (isEdit.value && form.value.id) {
    ...
    await http.post('/ffpp', form.value)
  }
